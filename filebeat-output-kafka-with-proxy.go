...
func (rd *randomDialer) Dial(network, addr string) (c net.Conn, err error) {
	for _, el := range rd.Proxies {
		if el.Key == addr {
			proxyURL, err := url.Parse(el.Url)
			if err != nil {
				// Logger.Printf("invalid proxy url format:%s", err)
				return nil, err
			}
			fmt.Printf("%s:连接代理===>%s\n", addr, el.Url)
	...
	return nil, errors.New("could not found proxies,check your configuration")
}
...
type kafkaConfig struct {
	...
	// proxies
	Proxies []socksProxy `config:"proxies"`
}
...
func newSaramaConfig(log *logp.Logger, config *kafkaConfig) (*sarama.Config, error) {
	...
	// configure network proxy properties
	if len(config.Proxies) > 0 {
		k.Net.Proxy.Enable = true
		dialer := &randomDialer{Proxies: config.Proxies}
		k.Net.Proxy.Dialer = dialer
	}
