...
|  ip地址   | 角色  | 主机名 |
|  ----  | ----  |----  |
| 172.17.27.17    | master,etcd-1 | k8s-master |
| 172.17.27.18  | node,etcd-2 | k8s-node-01 |
| 172.17.27.19  | node,etcd-3 | k8s-node-02 |
...
+ 基础设置
  ...
  ```console
  swapoff -a
  yum install conntrack-tools -y #aliyun 貌似需要安装
  ```
  ```cosnole
...
+ docker-ce 安装
 ...
 > etcd 部署
   ...
   cat > server-csr.json << EOF
   {
    ...
    #验证集群
    ETCDCTL_API=3  etcdctl --cacert=/etc/etcd/ssl/ca.pem --cert=/etc/etcd/ssl/server.pem --key=/etc/etcd/ssl/server-key.pem  \
--endpoints="https://172.17.27.17:2379,https://172.17.27.18:2379,https://172.17.27.19:2379" endpoint health --write-out=table
   ```
 ...
 > kube-apisever部署
   ...
   cat > ca-config.json << EOF
   {
    ...
    EOF
    #10.0.0.1 default service,10.0.0.2 coredns ip
    cat > server-csr.json << EOF
    {
        "CN": "kubernetes",
        "hosts": [
